...
asignedAcc <- read.delim("asignedAcc_pep.txt", header=FALSE, quote="", stringsAsFactors=FALSE,col.names = c("Method","SkTax","FamTax","Tax","Scientific.Name","Identity","Accession","ReadStatus","MapIdentity","MapPercent","MapReference"))
FamTaxNames <- read.delim("famtax_names_pep.txt",header=FALSE, quote="", stringsAsFactors=FALSE)
...
write.table(x = resultprotocol,file = paste(arbeitsvz,"/resultprotocol-complete-pep.txt",sep=""),quote = F,sep = "\t",dec = ".",row.names = F,na = "")
...
write.table(x = trimmed.length,file = paste(arbeitsvz,"/trimmed.length-pep.txt",sep=""),quote = F,sep = "\t",dec = ".",row.names = F,na = "")
write.table(x = raw.length,file = paste(arbeitsvz,"/raw.length-pep.txt",sep=""),quote = F,sep = "\t",dec = ".",row.names = F,na = "")
...
TaxIDs$Ident.tBlastx_vs_ntdb <- TaxIDs$tBlastx_vs_ntdb <-TaxIDs$Ident.Blastx_vs_protdb <- TaxIDs$Blastx_vs_protdb <-TaxIDs$Ident.Blastp_vs_protdb <- TaxIDs$Blastp_vs_protdb <- 0
...
write.table(x = TaxIDs,file = "resultprotocol-compact-pep.txt",quote = F,sep = "\t",dec = ".",row.names = F,na = "")
