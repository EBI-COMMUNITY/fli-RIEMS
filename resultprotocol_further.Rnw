...
completepep<- paste(projektname,"-resultprotocol-complete-pep.txt",sep="")
compactpep<- paste(projektname,"-resultprotocol-compact-pep.txt",sep="")
...
dateFurther1 <- as.character(Sys.getenv("dateFurther1"))
dateFurther2 <- as.character(Sys.getenv("dateFurther2"))
md5completepep <- as.character(Sys.getenv("md5completepep"))
md5compactpep <- as.character(Sys.getenv("md5compactpep"))
...
trimmed.length.pep <- read.table(file = "trimmed.length-pep.txt",quote = "",sep = "\t",dec = ".",na = "",header = T,stringsAsFactors = F)
raw.length.pep <- read.table(file = "raw.length-pep.txt",quote = "",sep = "\t",dec = ".",na = "",header = T,stringsAsFactors = F)
...
Further <- read.table("resultprotocol-compact-pep.txt",sep="\t",dec=".",header=T,stringsAsFactors = F, fill = T, comment.char="")
...
FamTaxNamesPep <- read.table("famtax_names_pep.txt",sep="\t",header=F,stringsAsFactors = F)
FamTaxNamesPep <- na.omit(FamTaxNamesPep)
...
wordcloud(words = c(header,header2,TaxIDs$Species,Further$Species),freq = c(freq,freq2,TaxIDs$counts, Further$counts),min.freq = 1,random.order = F,colors = farben,random.color = T,rot.per=.35, scale = c(25,5))
@
...
\lfoot{\Sexpr{projektname}-resultprotocol\_further.pdf}
...
\vspace{0.25cm}
\textbf{\textit{start of basic analysis:}} &  \Sexpr{date1} \\ 
\vspace{0.25cm} 
\textbf{\textit{end of basic analysis:}} & \Sexpr{date2} \\ 
\vspace{0.25cm}
\textbf{\textit{start of further analysis:}} &  \Sexpr{dateFurther1} \\ 
\vspace{0.25cm} 
\textbf{\textit{end of further analysis:}} & \Sexpr{dateFurther2} \\ 
\vspace{0.25cm}
\textbf{\textit{complete resultprotocol (basic):}} & \parbox[t]{10cm}{\Sexpr{complete}} \\ 
\vspace{0.25cm}
\textbf{\textit{md5sum:}}  & \Sexpr{md5complete} \\ 
\vspace{0.25cm}
\textbf{\textit{compact resultprotocol (basic):}}  & \parbox[t]{10cm}{\Sexpr{compact}} \\ 
\vspace{0.25cm}
\textbf{\textit{md5sum:}}  & \Sexpr{md5compact} \\ 
\vspace{0.25cm}
\textbf{\textit{complete resultprotocol (further):}} & \parbox[t]{10cm}{\Sexpr{completepep}} \\ 
\vspace{0.25cm}
\textbf{\textit{md5sum:}}  & \Sexpr{md5completepep} \\ 
\vspace{0.25cm}
\textbf{\textit{compact resultprotocol (further):}}  & \parbox[t]{10cm}{\Sexpr{compactpep}} \\ 
\vspace{0.25cm}
\textbf{\textit{md5sum:}}  & \Sexpr{md5compactpep} \\ 
\vspace{0.25cm}
...
sumf <- sum(Further$counts)
classifiedf <- as.character(format(sumf,big.mark=".", scientific = F))
@
...
\vspace{0.25cm}
\textbf{\textit{number of excluded reads (low quality / too short):}} & \Sexpr{as.character(lowqual)} \\ 
\vspace{0.25cm}
\textbf{\textit{number of reads classified by basic analysis:}} &  \Sexpr{as.character(classified)} \\ 
\vspace{0.25cm}
\textbf{\textit{number of reads classified by further analysis:}} &  \Sexpr{as.character(classifiedf)} \\ 
\vspace{0.25cm}
...
<<Histogram_Raw_Trimmed_unclassified_pep,echo=F,fig.height=6,fig.width=5,fig.align='center',warning=FALSE,message=FALSE,fig.cap='Read length distributions of the input dataset (upper panel) and the trimmed reads (lower).',fig.scap='Read length distributions (further analysis)',crop = TRUE, fig.pos='H'>>=

col <- sample(farben,1)

t.length <- cbind(trimmed.length.pep,2)
r.length <- cbind(raw.length.pep,1)

colnames(t.length) <- colnames(r.length) <- c("Length","Facet")
lengths <- as.data.frame(rbind(r.length,t.length),stringsAsFactors=F)
lengths$Facet <- factor(lengths$Facet)

lengths$Facet <- revalue(lengths$Facet,c("1"="Raw Reads","2"="Trimmed Reads"))
ggplot(lengths,aes(x=Length)) + geom_histogram(fill=col,colour="black") + facet_grid(Facet ~ .) + theme(plot.margin = unit(c(0,0,0,0), "cm"))
# + ggtitle("Read-Length-Histogram")
@
...
Part 1 of this section provides you with an overview of the classification results of the basic analysis. In figures \ref{fig:Superkingdom_TaxID} - \ref{fig:Species_TaxID} the classifications are summarised graphically at different taxonomic levels, and the most abundant species for each superkingdom are presented in table \ref{tab:\Sexpr{label1}} - \ref{tab:\Sexpr{label2}}. Part 2 provides an overview of the classification results of the further analysis. In figures \ref{fig:Superkingdom_TaxID_pep} and \ref{fig:Species_TaxID_pep} the classifications are summarised graphically at different taxonomic levels, and the most abundant species for each superkingdom are presented in table \ref{tab:common_species_pep}.

\subsection*{Part 1: Basic Analysis}
<<Superkingdom_TaxID, echo=F,comment="",warning=FALSE,message=FALSE,fig.height=6.5,fig.width=6.5,fig.align='center', fig.cap='Pie chart of percentages of the reads classified into superkingdoms. In the legend, the absolute number of reads classified into the respective taxa is given.', fig.pos='H', crop = TRUE>>=
...
for(i in uniqueSkTax){
	...
	print(table,include.rownames=FALSE,size="normalsize",caption.placement="top",tabular.environment="tabular")
...
\newpage
\newpage
\subsection*{Part 2: Further Analysis}

<<Superkingdom_TaxID_pep, echo=F,comment="",warning=FALSE,message=FALSE,fig.height=6.5,fig.width=6.5,fig.align='center', fig.cap='Pie chart of percentages of the reads classified into superkingdoms. In the legend, the absolute number of reads classified into the respective taxa is given.', fig.pos='H', crop = TRUE>>=

result_sk <- Further[,c(3,6)]
further_sk <- unique(Further$SkTax)
result_sk$Superkingdom <- 0
for( i in 1:nrow(SkTaxNames)){
	n <- which(result_sk$SkTax==SkTaxNames[i,1])
...
bp <- ggplot(counts,aes(x="",y=counts,fill=labels)) +geom_bar(width=1,stat="identity")
pie <- bp + coord_polar("y",start=90)
pie <- pie + scale_fill_manual(values = colour[which(colour$SkTax %in% further_sk),2]) + labs(fill="Superkingdom") + theme(plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"), legend.text=element_text(size=11,lineheight = 1), legend.key.height=unit(1,"cm"), legend.title=element_text(size=12), axis.title=element_blank(), axis.text.y=element_blank(), axis.text.x=element_blank(), axis.title.y=element_blank(), axis.ticks.y=element_blank())
pie 

@
The table \ref{tab:common_species_pep} provide information of the most abundant species and the families they belong to accompanied by the respective read counts and the NCBI taxonomy database IDs of superkingdom (SkTax), family (FamTax), and species (Tax). For further details of the assignments, please refer to table \ref{tab:compactresults_pep}.
<<The_most_common_Species_pep,echo=F,comment="",warning=FALSE,message=FALSE,results='asis',tidy=T>>=

uniqueSkTax <- unique(Further$SkTax)

	head <- Further[order(Further$counts,decreasing = T),]
	head <- head[,1:6]
	if(nrow(head) > 10) 
	{
		head <- head[1:10,]
    ...
    {
        famname <- FamTaxNamesPep[grep(head[i,4],FamTaxNamesPep[,1]),2]
        head[i,1] <- famname
    }
    
	caption <- as.character(paste("The most abundant species found by further analysis",".",sep=""))
	table <- xtable(head,caption=caption,label = paste("tab:common_species_pep"))
	align(table) <- c( "l", ">{\\itshape}p{1.25in} |", ">{\\itshape}p{2.5in}||", rep("c", 3),"|c")
	print(table,include.rownames=FALSE,size="normalsize",caption.placement="top",tabular.environment="longtable")

@

<<Species_TaxID_pep, echo=F,comment="",warning=FALSE,message=FALSE,fig.height=15,fig.width=15,fig.align='center', fig.cap='Graphical representation of read counts for the most abundant species. In order to represent all detected superkingdoms, the displayed species are selected from the complete result applying the HighestAverages algorithm from the R package SciencesPo. Species are sorted alphabetically, color-coded according to superkingdoms. Note that the graph is in log-scale.', crop = TRUE, fig.pos='H'>>=

result_tax <- Further[,c(3,2,6)]
result_tax$log.counts <- log(result_tax$counts)
result_tax$Superkingdom <- 0
for( i in 1:nrow(SkTaxNames)){
	n <- which(result_tax$SkTax==SkTaxNames[i,1])
...
This section provides you with a summary of all taxonomic assignments of all analyses. Table \ref{tab:compactresults} summarises the results of the basic analysis, table \ref{tab:compactresults_pep} those of the further analysis.
...
\newpage
%\newgeometry{left=1cm,right=1cm,bottom=1cm,top=1cm} % vielleicht ist es mit einer neueren version von geometry möglich dieses zu nutzen
<<resultprotocol_compact_pep,echo=F,results='asis',warnings=F,message=FALSE,tidy=T>>= 

coln <- colnames(Further)
coln <- gsub("_"," ",coln)
coln <- gsub("[.]", " ",coln)
colnames(Further) <- coln
caption <- as.character(c("Summary of all taxonomic assignments of the further analysis. The table is structured by taxonomic levels, starting with viruses, followed by bacteria and eukaryotes. In case no reads were classified into a superkingdom, the respective section is missing from the table. For all detected species, the scientific names as recorded in the NCBI taxonomy database and their classification into families is represented. The respective IDs from the NCBI taxonomy database are displayed (SkTax, superkingdom taxonomy ID; FamTax, family taxonomy ID; Tax, species taxonomy ID). If taxonomy IDs are missing, the respective species has not yet been fully classified. The total number of reads classified into the species is given in column ``counts''. In addition, the numbers of reads classified in the different analysis steps is provided. For all assignments, the range of sequence identities is provided next to the read counts.","Summary of all taxonomic assignments of the further analysis."))
table <- xtable(Further,caption=caption, label=paste("tab:compactresults_pep"))
align(table) <- c( "l",">{\\itshape}p{1.25in} | >{\\itshape}p{1.25in}||", rep("c", 3),"|c|", rep("c",ncol(Further)-5) )

positions <- list(-1)
j=2
for(i in unique(Further[,3])){
        if(i==11239){
        }else{
                positions[[j]] <- which(Further$SkTax == i)[1]-1
                j=j+1
        }
}

longtable.header <- "\\toprule\\\\"
for(i in 1:(ncol(Further)-1)){
	
	longtable.header <- paste(longtable.header,"{\\rotatebox{90}{\\parbox[t]{2cm}{\\raggedright\\textbf{",colnames(Further)[i]," }}}} &",collapse = "")
	
}
longtable.header <- paste(longtable.header,"{\\rotatebox{90}{\\parbox[t]{2cm}{\\raggedright\\textbf{",colnames(Further)[ncol(Further)]," }}}}  \\\\ \\midrule \\endfirsthead \\bottomrule",longtable.header,"{\\rotatebox{90}{\\parbox[t]{2cm}{\\raggedright\\textbf{",colnames(Further)[ncol(Further)]," }}}}  \\\\ \\midrule \\endhead \\bottomrule")
longtable.header <- gsub("_|[.]"," ",longtable.header)
