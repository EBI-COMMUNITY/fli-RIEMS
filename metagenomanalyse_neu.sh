...
if [[ ${embl_ebi} == "y" ]]
    then
        ...
        # Obtain the equivalent read accessions for the remaining accessions to be classified. Required due to differences in trimmomatic and 454
        while IFS= read -r line
            do
                grep $line restAcc.txt >> remainingAcc.txt
            done < restAccScaffold.txt
        cp remainingAcc.txt remainingAcc-IntermediaryCheck.txt
        mv remainingAcc.txt restAcc.txt
        trim=`wc -l < ${arbeitsvz}/asignedAcc.txt `                                                                     # count number of reads that were discarded after trimming
        untrim=`wc -l < ${arbeitsvz}/restAcc.txt `                                                                       # count number of total reads
        echo -ne "\n$(date) --> A total of $untrim Reads was added to the metagenomic workflow \n
        $trim Reads were marked as low quality Reads or have completed the Scaffold Assembly Blast stage. \n\n"

        exit
        # ADD A MAPPING STAGE HERE TO MAP REST READS
        # ENSURE THAT uniq-tids.txt is produced
        ...
        if [ -s ${arbeitsvz}/uniq-tids.txt ]                                                                            # if taxid were given in advance
            then
                ...
                echo -ne "$restreads\t$method\n" >> ${arbeitsvz}/report.txt
                duration=$SECONDS
                echo -ne "\n$(date) - Completed subset analysis."
                echo -ne "\nSTAGE TIMING\t$duration\tMegablast of contigs"
        fi
        echo -ne "\n===================== {Subset Analysis - End [`date`]} ====================="
fi

################################################################# Megablast der Rnd. Reads ###############################################################

echo -ne "\n\n===================== {Random Read Subset Megablast - Start [`date`]} ====================="
SECONDS=0
restreads=`wc -l ${arbeitsvz}/restAcc.txt | cut -d " " -f1`                                                     # count all remaing reads

START_8=$(date +%s)
echo -ne "\n[$(date)] Creating fasta file of 1000 randomly subset reads."
...
END_8=$(date +%s)
DIFF_8=$(( $END_8 - $START_8 ))
echo -ne "\nTIMING\t${DIFF_8}\tCreating fasta file of 1000 randomly subset reads"
...
duration=$SECONDS
echo -ne "\nSTAGE TIMING\t$duration\tMegablast of random reads"
echo -ne "\n===================== {Random Read Subset Megablast - End [`date`]} ====================="

################################################################# Mapping 2 #############################################################################

echo -ne "\n\n===================== {Mapping at Lower Cutoff - Start [`date`]} ====================="
    ...
    then
        ...
        while read line                                                                                         # while reading the references.txt file, do ...
            do
                if [ -s "$line" ]
                    then
                    echo -ne "\n[$(date)] ${line} exists and will be used for mapping at a lower cutoff."
                    ...
                    START_12=$(date +%s)
                    echo -ne "\n[$(date)] ${num} reads remaining to be classified..."
                    if (( $num > 20000 ))                                                                          # if more than 10000 reads are left, then ...
                        then
                            ...
                            echo -ne "\n[$(date)] Reference file ${name2} has size of ${size}."
                            ...
                            echo -ne "\n[$(date)] Splitting remaining accessions to run mapping in parallel on subsets of the remaining reads."
                            ...
                            echo -ne "\n[$(date)] Completed split of remaining accessions and mapping. Combined all separate results into one file."
                        else                                                                                        # if you have less than 10000 reads left, just run the mapping with all reads
                            echo -ne "\n[$(date)] Running mapping of reads against ${line} without need to split..."
                            ...
                            echo -ne "\n[$(date)] Running mapping of reads against ${line} without need to split... COMPLETE"
                    fi
                    END_12=$(date +%s)
                    DIFF_12=$(( $END_12 - $START_12 ))
                    echo -ne "\nTIMING\t${DIFF_12}\tMapping remaining reads at lower cutoff"

                    if [[ -s ${ziel}/Mapping2/454ReadStatus.txt ]]                                                                   # if you have a 454ReadStatus.txt in Mapping2 (Mapping was successful), then ...
                        then
                        START_13=$(date +%s)
                        get_mapping_results                                                                         # get the mapping results and assign them (see functions.sh)
                        END_13=$(date +%s)
                        DIFF_13=$(( $END_13 - $START_13 ))
                        echo -ne "\nTIMING\t${DIFF_13}\tObtaining lower cutoff mapping results and making read assignments"
                    fi
                fi
        ...
        echo -ne "\nSTAGE TIMING\t$duration\tMapping at lower cutoff\n"
fi           

echo -ne "$restreads\t$method\n" >> ${arbeitsvz}/report.txt                                                     # write restreads and mehtod to the report.txt
echo -ne "\n===================== {Mapping at Lower Cutoff - End [`date`]} ====================="

################################################################# Rest Assembly ##########################################################################

echo -ne "\n\n===================== {Assembling Remaining Reads - Start [`date`]} ====================="
SECONDS=0
...
until (( ${assfailcounter} > 2 ))                                                      # until you cannot assign any more reads or the assfailcounter is 2, do ...
    do
...
echo -ne "\nSTAGE TIMING\t$duration\tAssembling unassigned reads\n"
echo -ne "$restreads\t$method\n" >> ${arbeitsvz}/report.txt
echo -ne "\n===================== {Assembling Remaining Reads - End [`date`]} ====================="

############################################################### Identify all Orgs and download sequences ###################################################

echo -ne "\n\n===================== {Downloading Sequences of Identified Species - Start [`date`]} ====================="
SECONDS=0
...
while read line                                                                                                 # while reading the uniq_tid.txt, do...
    do
        ...
        if [[ `grep -w "$parentTax" ${arbeitsvz}/AnBlast.txt` == "" ]]                                          # if the found taxid is not already present in AnBlast.txt (all previous taxids are saved in the file), then ...
            then
                echo -ne "\n[$(date)] Taxonomic ID ${parentTax} was identified, searching for sequence files."
                ...
                if ls ${seq} 1> /dev/null 2>&1; then
                    ...
                    END_ADDSEQ=$(date +%s)
                    DIFF_ADDSEQ=$(( $END_ADDSEQ - $START_ADDSEQ ))
                    echo -ne "\nTIMING\t${DIFF_ADDSEQ}\tAdding sequence file to references and creating symlink"
                else
                     echo -ne "\nSequences not available for ${seq}, skipping organism."
                fi
        fi
    done < ${arbeitsvz}/uniq_tid.txt
END_24=$(date +%s)
DIFF_24=$(( $END_24 - $START_24 ))
echo -ne "\nTIMING\t${DIFF_24}\tIdentification of all organisms and downloading of sequences from uniq_tid.txt"
duration=$SECONDS
echo -ne "\nSTAGE TIMING\t$duration\tIdentification of all organisms and downloading of sequences\n"
echo -ne "\n===================== {Downloading Sequences of Identified Species - End [`date`]} ====================="

################################################################## Rest Megablast #########################################################################

echo -ne "\n\n===================== {Megablast against Nucleotide Database for Remaining Unassigned Reads - Start [`date`]} ====================="
SECONDS=0
...
END_25=$(date +%s)
DIFF_25=$(( $END_25 - $START_25 ))
echo -ne "\nTIMING\t${DIFF_25}\tRemaining reads megablast against nucleotide database\n"

START_26=$(date +%s)
getOutput                                                                                                    # get Output from blast and assign it to reads
END_26=$(date +%s)
DIFF_26=$(( $END_26 - $START_26 ))
echo -ne "\nTIMING\t${DIFF_26}\tPost processing and obtaining output of megablast against nt database for unassigned reads"

duration=$SECONDS
echo -ne "\nSTAGE TIMING\t$duration\tMegablast remaining reads against nt database"
echo -ne "$restreads\t$method\n" >> ${arbeitsvz}/report.txt
echo -ne "\n===================== {Megablast against Nucleotide Database for Remaining Unassigned Reads - End [`date`]} ====================="

################################################################ Blastn vs Orgs #######################################################################

echo -ne "\n\n===================== {Blastn Remaining Unassigned Reads against Identified Organisms - Start [`date`]} ====================="
SECONDS=0
...
END_27=$(date +%s)
DIFF_27=$(( $END_27 - $START_27 ))
echo -ne "\nTIMING\t${DIFF_27}\tCreating all organism BLAST database\n"
...
. ${installdir}/Blast.sh                                                                                    # call blast.sh
END_28=$(date +%s)
DIFF_28=$(( $END_28 - $START_28 ))
echo -ne "\nTIMING\t${DIFF_28}\tRemaining reads blastn against identified organisms\n"

START_29=$(date +%s)
getOutput                                                   # get Output from blast and assign it to reads
END_29=$(date +%s)
DIFF_29=$(( $END_29 - $START_29 ))
echo -ne "\nTIMING\t${DIFF_29}\tPost processing and obtaining output of blastn against identified organisms for unassigned reads"

duration=$SECONDS
echo -ne "\nSTAGE TIMING\t$duration\tBlastn remaining reads against identified organisms\n"
echo -ne "$restreads\t$method\n" >> ${arbeitsvz}/report.txt
echo -ne "\n===================== {Blastn Remaining Unassigned Reads against Identified Organisms - End [`date`]} ====================="


#################################################################### Rest Blastn #########################################################################

echo -ne "\n\n===================== {Blastn of Remaining Reads against Nucleotide Database - Start [`date`]} ====================="
SECONDS=0
...
. ${installdir}/Blast.sh                                                                                    # call blast.sh
END_30=$(date +%s)
DIFF_30=$(( $END_30 - $START_30 ))
echo -ne "\nTIMING\t${DIFF_30}\tRemaining reads blastn against nucleotide database\n"

START_31=$(date +%s)
getOutput                           # get Output from blast and assign it to reads# get Output from blast and assign it to reads
END_31=$(date +%s)
DIFF_31=$(( $END_31 - $START_31 ))
echo -ne "\nTIMING\t${DIFF_31}\tPost processing and obtaining output of blastn against nt database"

duration=$SECONDS
echo -ne "\nSTAGE TIMING\t$duration\tBlastn against nt database\n"
echo -ne "$restreads\t$method\n" >> ${arbeitsvz}/report.txt
echo -ne "\n===================== {Blastn of Remaining Reads against Nucleotide Database - End [`date`]} ====================="


#################################################################### finish Analysis #########################################################################

echo -ne "\n\n===================== {Consolidating Analysis - Start [`date`]} ====================="
...
END_32=$(date +%s)
DIFF_32=$(( $END_32 - $START_32 ))
echo -ne "\nTIMING\t${DIFF_32}\tInitial final analysis processing"
...
END_33=$(date +%s)
DIFF_33=$(( $END_33 - $START_33 ))
echo -ne "\nTIMING\t${DIFF_33}\tCreating result procotol PDF"

echo -ne "\nDone\nCompleted basic analysis --> $(date)"                                                                                               # user info
echo -ne "\n===================== {Consolidating Analysis - End [`date`]} ====================="
